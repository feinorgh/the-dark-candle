...
| 7 | `material_at_radius` — depth bins, soil, sea-level | `src/gpu/shaders/voxel_gen.wgsl:624-670` | `src/world/terrain.rs:754-849` | divergent | yes | yes | GPU uses `r_offset` (from mean_radius) to avoid Earth-scale f32 cancellation. CPU uses absolute `r`. Depth logic matches. |
