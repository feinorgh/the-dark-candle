...
### Underwater Caustics ✅ (Phase 3.3, hybrid approach)

Voxel terrain now renders an underwater shimmery caustic contribution
through a Bevy 0.18 `ExtendedMaterial<StandardMaterial, TerrainCausticExt>`
wrapper, replacing the three plain `StandardMaterial` cache sites in
`src/world/v2/chunk_manager.rs` (chunk faces),
`src/world/v2/stitch_mesh.rs` (pairwise LOD seams and corner caps).

All three materials share a single uniform refreshed in `PostUpdate`
after `TransformSystems::Propagate` via
`update_terrain_caustic_uniform` (`src/lighting/terrain_caustic_material.rs`),
guaranteeing the sun direction, planet centre (in render-space, taking
floating-origin into account) and sea-level radius are consistent
across all three handles every frame.

The fragment shader (`assets/shaders/terrain_caustic.wgsl`) only
contributes when:

- `depth = sea_level_radius − |world_pos − planet_centre| > 0`
  (smoothstep-masked over the first 2 m to avoid f32 precision
  stipple at the waterline),
- `cos(zenith) = up · sun_dir > 0` (no caustic at night), and
- the Beer–Lambert attenuation `exp(−falloff × depth)` is non-trivial.

The pattern itself is a 2-octave procedural Worley `F2 − F1` field
sharpened with `pow(.., 6 | 8)` and shifted to zero mean so the
caustic only adds sparkle — it does not double-count the diffuse
sunlight the StandardMaterial PBR pass already produces. The CPU
oracle in `src/lighting/caustic_tile.rs` documents the same algorithm
under five unit tests (non-empty, zero mean, bounded peak, seamless
wrap, seed independence).

Forward rendering is forced (`opaque_render_method = Forward`) so
the post-lighting addition is preserved — the deferred path of
ExtendedMaterial can only modify `PbrInput`, not the final colour.

Phase 3.3 lands the *render-side* scaffolding and a procedural
caustic pattern; the physical baked LUT generated by
`src/lighting/caustics.rs` photon tracer is deferred until that
module gains wave-surface support (a flat refractive interface
produces a uniform brightness boost, not a caustic web).

### Aurora Borealis / Australis ✅ (Phase 3.5)

Polar auroral emission rendered as a volumetric outer-shell sphere mesh with a
custom additive `AuroraMaterial` backed by `assets/shaders/aurora.wgsl`.

**CPU side** (`src/lighting/aurora.rs`):

- `magnetic_north_axis(planet_north_axis, lat_offset_deg, lon_offset_deg)` —
  builds a tilted magnetic dipole axis from geographic north using an
  orthonormal basis, matching Earth's ~11.5° geomagnetic tilt. Fully tested.
- `aurora_band_mask(cos_mlat, band_center_rad, half_width_rad)` — smooth
  auroral oval envelope: smoothstep rise from equator-side, flat plateau,
  smoothstep fall to pole-side. Zero at equator, peak near 65–70° MLAT.
- `aurora_day_side_factor(cos_sun_dot_up)` — suppresses emission on the
  sunlit hemisphere (factor → 0 in daytime, → 1 at night). Avoids the
  visible-in-daylight artefact.
- `spawn_aurora_shell` (Startup) — spawns a sphere at
  `r_inner = 6_471 km`, `r_outer = 6_571 km` with `NoFrustumCulling` so
  the shell is always submitted to the GPU regardless of the camera frustum.
- `anchor_aurora_shell_to_planet` (PostUpdate) — keeps the shell centred on
  the planet origin in render-space by reading `RenderOrigin`.
- `update_aurora_material` (Update) — pushes `planet_center_render`,
  `magnetic_north_axis`, `sun_world_direction`, `params`, and animated
  `band.w` (curtain oscillation speed) to the GPU uniform buffer each frame.

**GPU side** (`assets/shaders/aurora.wgsl`):

The fragment shader ray-marches through the auroral shell (8 steps, early-exit
when opacity saturates). At each sample it evaluates:

- Magnetic latitude from the dipole axis dot product.
- Band mask × day-side factor → emission weight.
- Curtain pattern: longitude-aligned sine wave (`sin(lon × freq + time × speed)`)
  shaped by `pow(max(0, ...), 3)` for sharp, bright curtain columns.
- Height falloff: `smoothstep` from `r_inner` to `r_outer`.
- Colour: green core (`#00ff44`) blending to magenta fringe (`#ff44cc`)
  across the sample height.
- Additive alpha blend (`AlphaMode::Add`) so the aurora composites on top of
  the night sky without overdrawing stars.

**Visual evidence** (agent captures, commit alongside):

| Capture | Location | Expected | File size |
|---|---|---|---|
| surface-night-pole | 70°N, 0°E, pitch +30° | Aurora band visible | ~437 KB |
| surface-day-pole | 70°N, 180°E, pitch +30° | Aurora suppressed | ~438 KB |
| surface-night-equator | 0°, 0°, pitch +30° | No aurora (band mask=0) | ~417 KB |
| orbital-nadir | 70°N, 0°E, pitch −90° | Looking down at terrain | ~424 KB |

**Deferred** (tracked in `AURORA-001`): storm-coupled intensity, non-dipolar /
quadrupolar field components, multi-band Jovian-style rings, dynamic pole
migration. True orbital altitude spawn is also deferred pending CLI support.
